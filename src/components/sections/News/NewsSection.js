...
import {newsPage} from '@/lib/mock-data';
...
export default function NewsSection({items = []}) {
    ...
    const pageCount = Math.max(1, Math.ceil(filtered.length / newsPage.pageSize));
    const safePage = Math.min(page, pageCount);
    const visible = filtered.slice(
        (safePage - 1) * newsPage.pageSize,
        safePage * newsPage.pageSize,
    );
